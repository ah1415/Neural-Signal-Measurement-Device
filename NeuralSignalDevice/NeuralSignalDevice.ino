...
void adcInit(metadata_t* meta) {
  ...
  ble.print(F("Sample pins:"));
  for (uint8_t i = 0; i < meta->pinCount; i++) {
    ble.print(' ');
    ble.print(meta->pinNumber[i], DEC);
  }
  ble.println();
  ble.print(F("ADC bits: "));
  ble.println(meta->recordEightBits ? 8 : 10);
  ble.print(F("ADC clock kHz: "));
  ble.println(meta->adcFrequency/1000);
  ble.print(F("Sample Rate: "));
  ble.println(sampleRate);
  ble.print(F("Sample interval usec: "));
  ble.println(1000000.0/sampleRate, 4);
}
//------------------------------------------------------------------------------
...
void binaryToCsv() {
  ...
  if (!binFile.isOpen()) {
    ble.println(F("No current binary file"));
    return;
  }
  binFile.rewind();
  ...
  ble.print(F("Writing: "));
  ble.print(csvName);
  ble.println(F(" - type any character to stop"));
  pm = (metadata_t*)&buf;
  ...
  while (!ble.available() && binFile.read(&buf, 512) == 512) {
    if (buf.count == 0) {
      break;
    }
    if (buf.overrun) {
  ...
  csvStream.fclose();
  ble.print(F("Done: "));
  ble.print(0.001*(millis() - t0));
  ble.println(F(" Seconds"));
}
//------------------------------------------------------------------------------
...
void checkOverrun() {
  ...
  binFile.rewind();
  ble.println();
  ble.println(F("Checking overrun errors - type any character to stop"));
  if (!binFile.read(&buf , 512) == 512) {
    error("Read metadata failed");
  }
  bn++;
  while (binFile.read(&buf, 512) == 512) {
    ...
    if (buf.overrun) {
      if (!headerPrinted) {
        ble.println();
        ble.println(F("Overruns:"));
        ble.println(F("fileBlockNumber,sdBlockNumber,overrunCount"));
        headerPrinted = true;
      }
      ble.print(bn);
      ble.print(',');
      ble.print(bgnBlock + bn);
      ble.print(',');
      ble.println(buf.overrun);
    }
    bn++;
  }
  if (!headerPrinted) {
    ble.println(F("No errors found"));
  } else {
    ble.println(F("Done"));
  }
}
...
void logData() {
  ...
  if (sd.exists(TMP_FILE_NAME)) {
    ble.println(F("Deleting tmp file"));
    if (!sd.remove(TMP_FILE_NAME)) {
      error("Can't remove tmp file");
    }
  }
  // Create new file.
  ble.println(F("Creating new file"));
  binFile.close();
  ...
  ble.println(F("Erasing all data"));
  ...
  delay(1000);
  ble.println(F("Logging - type any character to stop"));
  ...
  ble.print(F("File renamed: "));
  ble.println(binName);
  ble.print(F("Max block write usec: "));
  ble.println(maxLatency);
  ble.print(F("Record time sec: "));
  ble.println(0.001*(t1 - t0), 3);
  ble.print(F("Sample count: "));
  ble.println(count/PIN_COUNT);
  ble.print(F("Samples/sec: "));
  ble.println((1000.0/PIN_COUNT)*count/(t1-t0));
  ble.print(F("Overruns: "));
  ble.println(overruns);
  ble.println(F("Done"));
}
//------------------------------------------------------------------------------
void setup(void) {
  ...
  ble.print(F("FreeStack: "));
  ble.println(FreeStack());
...
void loop(void) {
  ...
  do {
    delay(10);
  } while (ble.available() && ble.read() >= 0);
  ble.println();
  ble.println(F("type:"));
  ble.println(F("c - convert file to csv"));
  ble.println(F("d - dump data to Serial"));
  ble.println(F("e - overrun error details"));
  ble.println(F("r - record ADC data"));
